...
static inline void convertImageData(tinygltf::Image& image, void* &data);
...
struct ImageParseTaskSet : enki::ITaskSet {
    ImageParseTaskSet(std::vector<tinygltf::Image>& images, std::vector<void*>& buffers)
    : m_Images(images), m_Buffers(buffers) { m_SetSize = (uint32_t)m_Images.size(); }
    ...
    void ExecuteRange( enki::TaskSetPartition range_, uint32_t threadnum_ ) override {
        for(unsigned int i = range_.start; i < range_.end; ++i ) {
            convertImageData(m_Images.at(i), m_Buffers.at(i));
        }
    }
};
...
m_Physics(init.physics), m_Assets(init.assets), m_Cameras(init.cameras), m_Lights(init.lights), m_NodeTree(init.nodeTree), m_FilePath(filePath) {
    ...
    loadMaterials();
    m_StbBuffers.resize(m_gltfModel.images.size());
    

    sw.begin();
    
    ImageParseTaskSet task(m_gltfModel.images, m_StbBuffers);
    g_TaskScheduler.AddTaskSetToPipe(&task);
    loadTextures();
    g_TaskScheduler.WaitforTask(&task);
    
    sw.end("Loading textures: ");
}

void ImportGLTF::loadTextures(void) {
    size_t offset = m_Assets.textures.size();
    m_Assets.textures.resize(offset + m_gltfModel.images.size());
    
    size_t numImages = m_StbBuffers.size();
    VkSamplerCreateInfo samplerInfo = {};
    
    while(numImages > 0)
    for (auto& tex : m_gltfModel.textures) {
        if (m_Assets.textures.at(offset + tex.source) == nullptr && m_StbBuffers.at(tex.source) != NULL) {
            const tinygltf::Image& image = m_gltfModel.images.at(tex.source);
            
            fillSamplerInfo(tex.sampler, &samplerInfo);
            
            m_Assets.textures.at(offset + tex.source) =
            std::make_unique<const Texture>(m_Device,
                                            m_Image,
                                            m_StbBuffers.at(tex.source),
                                            image.width,
                                            image.height,
                                            image.component,
                                            true,
                                            getVulkanFormat(image.bits, image.component),
                                            &samplerInfo);
            
            stbi_image_free(m_StbBuffers.at(tex.source));
            --numImages;
        }
    }
}
...
void ImportGLTF::parseGLTF(void) {
...
void ImportGLTF::loadNodeFromModel(int gltfIndex, uint32_t parentIndex) {
...
void ImportGLTF::loadMaterials(void) {
    size_t offset = m_Assets.textures.size();
    ...
    for (const tinygltf::Material &gltfMaterial : m_gltfModel.materials) {
        ...
        if (colorTextureIndex > -1) {
            int bufferIndex = m_gltfModel.textures[colorTextureIndex].source;

            material.setColorTexture(offset + bufferIndex);
            material.setNormalTexCoordSet(gltfMaterial.pbrMetallicRoughness.baseColorTexture.texCoord);
            
        }
        
        if (normalTextureIndex > -1) {
            int bufferIndex = m_gltfModel.textures[normalTextureIndex].source;

            material.setNormalTexture(offset + bufferIndex);
        ...
        if (metalRoughTextureIndex > -1) {
            int bufferIndex = m_gltfModel.textures[metalRoughTextureIndex].source;

            material.setRoughMetalTexture(offset + bufferIndex);
        ...
        if (occlusionTextureIndex > -1) {
            int bufferIndex = m_gltfModel.textures[occlusionTextureIndex].source;

            if (occlusionTextureIndex == metalRoughTextureIndex) {
                material.setARMTexture(offset + bufferIndex);
            } else {
                material.setOcclusionTexture(offset + bufferIndex);
            }
...
void ImportGLTF::fillSamplerInfo(int textureIndex, VkSamplerCreateInfo *samplerInfo) {
...
void ImportGLTF::parsePhysicsMaterialsAndShapes(void) {
...
static inline void convertImageData(tinygltf::Image& image, void* &data) {
    ...
    unsigned char* stbi_alloc = NULL;
    
    int bits = 8;
    int pixel_type = TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE;
    
    if (stbi_is_16_bit_from_memory(image.image.data(), (int)image.image.size())) {
        stbi_alloc = reinterpret_cast<unsigned char *>(stbi_load_16_from_memory(image.image.data(), (int)image.image.size(), &w, &h, &comp, req_comp));
        if (data) {
            bits = 16;
    ...
    if (!stbi_alloc) stbi_alloc = stbi_load_from_memory(image.image.data(), (int)image.image.size(), &w, &h, &comp, req_comp);
    
    if ((w < 1) || (h < 1)) {
        stbi_image_free(stbi_alloc);
        return;
    }
